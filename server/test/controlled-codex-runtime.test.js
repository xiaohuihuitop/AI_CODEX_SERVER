...
test('受控运行时不按官方客户端版本号阻断结构兼容的页面', async () => {
  const cdp = createCdp();
  const runtime = new ControlledCodexRuntime({
    debugPort: 9230,
    reader: {},
    ...
    controller: { probeCompatibility: async () => ({ compatible: true }) },
    portOwnerResolver: async () => ({ pid: 10 }),
    cdpProbe: async () => ({ ok: true }),
  });

  assert.deepEqual(await runtime.start(), { debugPort: 9230, pid: 10, version: '26.800.1.0' });
  assert.equal(runtime.state, 'ready');
  runtime.stopRuntime();
});
