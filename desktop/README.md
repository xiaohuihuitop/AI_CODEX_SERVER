...
- 官方 Codex 未运行且配置端口空闲时，重启按钮会直接启动官方客户端并启用 CDP，不要求先手动打开 Codex。
- 官方 Codex 已通过 CDP 连接时，重启按钮先发送标准 `Browser.close` 优雅退出，再重新激活；只有旧实例没有可用 CDP 时才终止已核对的官方进程树。
- 端口有监听但对应 PID 已不存在时，管理器会明确报告系统残留监听。该状态不能安全终止未知进程，也不会自动换端口；需重启 Windows 释放端口，或手动修改 CDP 端口。
...
- CDP 实际可访问时直接复用，不依赖 Windows Appx 主进程命令行是否显示启动参数。
