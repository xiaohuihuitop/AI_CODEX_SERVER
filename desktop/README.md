...
- Agent 初次接管失败后会保持运行并定时重试，不再退出为“Agent 未运行”；同一 Agent 生命周期只允许执行一次可能关闭官方客户端的受控启动。
- CDP 实际可访问时直接复用，不依赖 Windows Appx 主进程命令行是否显示启动参数。需要首次启用 CDP 时仅请求官方窗口正常关闭，不再强制终止进程。
