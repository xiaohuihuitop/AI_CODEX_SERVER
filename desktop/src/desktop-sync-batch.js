 ...
 * @param {Array<string>} changedThreadIds JSONL 已变化的线程标识。
 * @returns {{targets: Array<object>, nextCursor: number, prioritized: boolean, priorityReason?: string}} 选中的线程和下一个常规游标。
 */
function selectSyncBatch(targets, cursor, batchSize, priorityThreadId = '', changedThreadIds = []) {
  ...
  if (priority) return { targets: [priority], nextCursor: start, prioritized: true, priorityReason: 'control' };

  const count = Math.max(1, Math.min(Number(batchSize) || 1, length));
  const changed = new Set((changedThreadIds || []).map(item => String(item || '').trim()).filter(Boolean));
  if (changed.size) {
    const changedTargets = rows.filter(target => changed.has(target.threadId)).slice(0, count);
    if (changedTargets.length) return { targets: changedTargets, nextCursor: start, prioritized: true, priorityReason: 'changed' };
  }
  const batch = [];
